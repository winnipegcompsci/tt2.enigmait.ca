<div class="actions columns col-lg-2 col-md-3 pull-right">
    <h3><?= __('Actions') ?></h3>
    <ul class="side-nav">
        <li><?= $this->Html->link(__('Edit Ticket Event Type'), ['action' => 'edit', $ticketEventType->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Ticket Event Type'), ['action' => 'delete', $ticketEventType->id], ['confirm' => __('Are you sure you want to delete # {0}?', $ticketEventType->id)]) ?> </li>
...
<div class="ticketEventTypes view col-lg-10 col-md-9 columns">
    <h2><?= h($ticketEventType->name) ?></h2>
    <div class="row">
        <div class="large-5 columns strings">
            <h6 class="subheader"><?= __('Name') ?></h6>
            <p><?= h($ticketEventType->name) ?></p>
        </div>
        <div class="large-2 columns numbers end">
            <h6 class="subheader"><?= __('Id') ?></h6>
            <p><?= $this->Number->format($ticketEventType->id) ?></p>
            <h6 class="subheader"><?= __('Order') ?></h6>
            <p><?= $this->Number->format($ticketEventType->order) ?></p>
        </div>
    </div>
</div>
<div class="related row">
    ...
    <?php if (!empty($ticketEventType->ticket_events)): ?>
    <table cellpadding="0" cellspacing="0">
        ...
        <?php foreach ($ticketEventType->ticket_events as $ticketEvents): ?>
        <tr>
