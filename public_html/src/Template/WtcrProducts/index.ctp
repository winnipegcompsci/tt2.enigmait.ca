...
<div class="wtcrProducts index col-lg-10 col-md-9 columns">
    ...
    <?php foreach ($wtcrProducts as $wtcrProduct): ?>
        <tr>
            ...
            <td>
                <?= $wtcrProduct->has('wtcr_vendor') ? $this->Html->link($wtcrProduct->wtcr_vendor->name, ['controller' => 'WtcrVendors', 'action' => 'view', $wtcrProduct->wtcr_vendor->id]) : '' ?>
            </td>
            ...
            <td><?= h($wtcrProduct->description) ?></td>
            <td><?= h($wtcrProduct->wtcr_vendor_sku) ?></td>
            <td class="actions">
