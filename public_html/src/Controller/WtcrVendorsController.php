        ...
        while($index < count($pno)) {
            ...
            if(strcmp($prices[$index], "0.00") !== 0) {
                ...
                if($vendorProducts->save($thisProduct)) {
                    ...
                    echo "<br />Request Data: <pre>" . print_r($this->request->data(), TRUE) . "</pre>";
    ...
    public function fetch_asi_products() {
        ...
        foreach($productTypes as $key => $productType) {            
            ...
            for($pos = 0; $pos < count($product_skus); $pos++) {
                ...
                if($vendorProducts->save($thisProduct)) {
                    error_log('Saved #' . $thisProduct->id . '::' . $thisProduct->mfg_part_num . ':: ' . $product_names[$pos] . ' with Product Number: ' . $product_skus[$pos]);
                } else {
